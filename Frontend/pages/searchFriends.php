...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['query'])) {
    $results = rmq_rpc('friends.search', ['user_id' => $_SESSION['id'], 'query' => trim($_GET['query'])])['results'] ?? [];
}
...
<body>
    ...
    <form method="GET" action="searchFriends.php" class="mb-4">
        ...
        <ul class="list-group">
            <?php foreach ($results as $user): ?>
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <a href="friendActions.php?action=add&user_id=<?= urlencode($user['id']) ?>" class="btn btn-sm btn-success">Add Friend</a>
                </li>
            <?php endforeach; ?>
        </ul>
    <?php elseif (isset($_GET['query'])): ?>
        <p>No users found matching "<?= htmlspecialchars($_GET['query']) ?>". Try a different search?</p>
    <?php endif; ?>
</body>
